...
sem_t rwsem;
// 记录登录状态
atomic_bool g_isLoginSuccess{false}; // 在多个线程中处理的变量定义成原子类型
...
int main(int argc, char **argv)
{
    ...
    while (1)
    {
        ...
        { // login
            ...
            sem_wait(&rwsem); // 等待子线程处理完登录的响应消息后通知
        ...
        { // register
            ...
            sem_wait(&rwsem); // 等待子线程处理完注册的响应消息后通知
...
void doRegResponse(json &responsejs)
{
    if (0 != responsejs["errno"]) // 注册失败
        cerr << "the name is already exist, register error!" << endl;
    else
